...
class PostgreSQL(StorageProvider):
    ...
    def __getattribute__(self, name):
        if (
            ...
            or self.is_open
            or name.endswith(
                ("__db_name", "__db_user", "__db_password", "__db_port")
            )
        ):
            return object.__getattribute__(self, name)
        error = "{} is not available since the connection is closed".format(
            name
        )
    ...
    def get_app_user_by_username(self, username):
        ...
        parms = (username,)
        error = "can not get app_user (username={}), reason: ".format(
            username
        )
    ...
    def exists_app_username(self, username):
        ...
        parms = (username,)
        error = "can not check if app_user exists (username={}), reason: ".format(
            username
        )
    ...
    def add_post(self, post):
        ...
        error = "can not add post (oid={}, title={}), reason: ".format(
            post.oid, post.title
        )
        self._execute_process_sql(sql, parms, error)
