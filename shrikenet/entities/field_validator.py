...
class FieldValidator:
    ...
    tag_min_length = 4
    tag_max_length = 50
    tag_regex_pattern = "^[a-z0-9][a-z0-9_]*[a-z0-9]$"
    ...
    def validate_int(value, field_name="value", lower_limit=0):
        ...
        if not isinstance(value, int) or value < lower_limit:
            raise ValueError(
                "{0} must be an integer greater than or equal "
    ...
    def validate_username(username, field_name="username"):
        return FieldValidator.validate_string(
            ...
            pattern_hint=(
                field_name + " must be alphanumeric characters with "
    ...
    def validate_string(
        ...
        if len(field_value) < min_length or len(field_value) > max_length:
            raise ValueError(
                "{0} must be between {1} and {2} characters long".format(
                    field_name, min_length, max_length
                )
            )
        if field_value.startswith(" "):
            raise ValueError(
                "{} must not have leading spaces".format(field_name)
            )
        if field_value.endswith(" "):
            raise ValueError(
                "{} must not have trailing spaces".format(field_name)
            )
    ...
    def validate_name(name, field_name="name"):
        return FieldValidator.validate_string(
            ...
            pattern_hint=(
                field_name + " must be alphanumeric characters "
    ...
    def validate_title(title, field_name="title"):
        return FieldValidator.validate_string(
            ...
            pattern_hint=(
                "{} must begin with an alphanumeric character".format(
                    field_name
                )
            ),
        )
    ...
    def validate_tag(tag, field_name="tag"):
        return FieldValidator.validate_string(
            ...
            regex_pattern=FieldValidator.tag_regex_pattern,
            pattern_hint=(
                field_name + " must be alphanumeric characters "
                "in lowercase with optional underscore seperators"
            ),
        )
    ...
    def validate_instant(instant, field_name="instant"):
        ...
        if not isinstance(instant, datetime) or instant.tzinfo is None:
            raise ValueError(
                "{} must be datetime object with timezone".format(
                    field_name
                )
            )
        return instant
